...
def predict():
    ...
    try:
        ...
        # Convert input to string format used in training
        it_text = " ".join(it_skills)
        ...
        des_seq = des_tokenizer.texts_to_sequences([designation])

        # Pad sequences
        X_it = pad_sequences(it_seq, maxlen=MAX_IT_LEN, padding="post")
        X_soft = pad_sequences(soft_seq, maxlen=MAX_SOFT_LEN, padding="post")

        # Designation handling (same as training: seq[0] - 1)
        if len(des_seq[0]) > 0:
            X_des = np.array([[des_seq[0][0] - 1]])
        else:
            X_des = np.array([[0]])
